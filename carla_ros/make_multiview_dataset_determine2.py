...
def Spawn_the_vehicles(world,client,car_num,tmport):
    ...
    traffic_manager = client.get_trafficmanager(args.tm_port+tmport)
...
def multipro():
    ...
    for i,sensor_spec in enumerate(global_sensors):
        ...
        p2 = multiprocessing.Process(target=main,args=(spawn_point.pop("x"),-spawn_point.pop("y"),spawn_point.pop("z"),
                -spawn_point.pop("pitch", 0.0),-spawn_point.pop("yaw", 0.0),spawn_point.pop("roll", 0.0),sensor_id,i,global_file))
        #p1.start()
        #time.sleep(5)
        global_file = False
        p2.start()
