...
if __name__ == '__main__':
    ...
    #遍历融合
    import carla_ros.utils as uti
    import cv2
    def vehicle_world2sensor(vehicles,cam_trans,yaw):
        ret = []
        cam_trans = uti.ClientSideBoundingBoxes.get_matrix(cam_trans)
        for car in vehicles:
            cord_p = np.zeros((1,4))
            cord_p[0][0] = car[2]
            cord_p[0][1] = car[0]
            cord_p[0][2] = -car[1]
            cord_p[0][3] = 1
            rotation_y = car[6] * 180 / np.pi

            p_in_cam1 = np.dot(np.linalg.inv(cam_trans), np.transpose(cord_p))
            ry_world2cam1 = (rotation_y - yaw+90) * np.pi / 180
            tv = uti.CamVehicle(p_in_cam1[1][0],-p_in_cam1[2][0],p_in_cam1[0][0],car[5],car[4],car[3],ry_world2cam1)
            ret.append(tv)
        return ret
    ...
    fused_data = cu.matching_and_fusion([cam_test_list[2][44]],[cam_test_list[19][44]]) #融合
    fused_data = cu.matching_and_fusion(fused_data,[cam_test_list[33][44]]) #融合
    vehicls = vehicle_world2sensor(fused_data[0],cam_transform[2],-20)
    box3d_list = [car.compute_box_3d() for car in vehicls]
    #print(box3d_list)
    bird_view = uti.add_bird_view(box3d_list)
