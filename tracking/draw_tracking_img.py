...
if __name__ == "__main__":
    ...
    for pred in detect_main_list:
        ...
        vehicles = get_vehicle_list(anns,cam_transform['cam9'])
        ...
        #     vehicles2 = get_vehicle_list(anns2,cam_transform['cam9'])
            ...
            #         id_list.append(cid)
        v_list = vehicle_world2sensor(vehicles,cam_transform['cam9'],-30)
        box3d_list = [car.compute_box_3d() for car in v_list]
        id_list = [car.id for car in v_list]
        color_list = [tuple([int(tmp * 255) for tmp in colors[obj.id % 30]]) for obj in v_list]
        
        #print(box3d_list)
        bird_view = add_bird_view(box3d_list)
        cv2.imshow('bird',bird_view)
        image = cv2.imread('/home/ubuntu/xwp/datasets/multi_view_dataset/new/cam_sample/image_2/008945.png')
        ...
        for i,(box3d,oid) in enumerate(zip(box3d_list,id_list)):
            #print(box_3d)
            color = tuple([int(tmp * 255) for tmp in colors[oid % 30]])
